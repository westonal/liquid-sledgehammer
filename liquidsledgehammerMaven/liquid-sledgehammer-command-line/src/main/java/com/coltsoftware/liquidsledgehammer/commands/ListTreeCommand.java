...
public final class ListTreeCommand extends PathCommandBase implements Command {
    ...
    @Override
    public ArgumentCompleter getCompleter(State state) {
        return new ArgumentCompleter() {
            @Override
            public int complete(String buffer, int cursor, List<CharSequence> candidates) {
                System.out.println(buffer);
                findPath(System.out, state.getCurrentNode(), buffer);
                for (FinancialTreeNode node : state.getCurrentNode()) {
                    int currentPathLength = state.getCurrentNode().getFullPath().length();
                    String candidate = node.getFullPath().substring(currentPathLength + 1);
